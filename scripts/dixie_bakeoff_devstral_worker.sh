...
VLLM_LOG=/var/log/vllm-devstral.log
...
cleanup() {
  ...
  aws s3 cp "$VLLM_LOG" "$PREFIX/logs/vllm-devstral.log" --only-show-errors || true
  sync
...
MODEL_WEIGHTS='mistralai/Devstral-Small-2-24B-Instruct-2512'
SERVED_MODEL='mistralai/Devstral-Small-2-24B-Instruct-2512'
...
export DIXIE_BAKEOFF_DEVSTRAL_API_BASE=http://127.0.0.1:8000/v1
mkdir -p output
for repetition in 1 2 3; do
  .venv/bin/dixie bakeoff configs/model_bakeoff.yaml \
    --candidate devstral-small-24b \
    --output-dir "output/devstral-small-24b/run-${repetition}"
done
