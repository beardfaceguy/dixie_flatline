...
VLLM_LOG=/var/log/vllm-mistral.log
...
cleanup() {
  ...
  aws s3 cp "$VLLM_LOG" "$PREFIX/logs/vllm-mistral.log" --only-show-errors || true
  sync
...
MODEL_WEIGHTS='mistralai/Mistral-7B-Instruct-v0.3'
SERVED_MODEL='mistralai/Mistral-7B-Instruct-v0.3'
...
export DIXIE_BAKEOFF_MISTRAL_API_BASE=http://127.0.0.1:8000/v1
mkdir -p output
for repetition in 1 2 3; do
  .venv/bin/dixie bakeoff configs/model_bakeoff.yaml \
    --candidate mistral-7b-control \
    --output-dir "output/mistral-7b-control/run-${repetition}"
done
