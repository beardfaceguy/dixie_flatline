...
VLLM_LOG=/var/log/vllm-redsage.log
...
cleanup() {
  ...
  aws s3 cp "$VLLM_LOG" "$PREFIX/logs/vllm-redsage.log" --only-show-errors || true
  sync
...
MODEL_WEIGHTS='RISys-Lab/RedSage-Qwen3-8B-DPO'
SERVED_MODEL='RISys-Lab/RedSage-Qwen3-8B-DPO'
...
export DIXIE_BAKEOFF_REDSAGE_API_BASE=http://127.0.0.1:8000/v1
mkdir -p output
for repetition in 1 2 3; do
  .venv/bin/dixie bakeoff configs/model_bakeoff.yaml \
    --candidate redsage-8b \
    --output-dir "output/redsage-8b/run-${repetition}"
done
