...
VLLM_LOG=/var/log/vllm-gpt-oss.log
...
cleanup() {
  ...
  aws s3 cp "$VLLM_LOG" "$PREFIX/logs/vllm-gpt-oss.log" --only-show-errors || true
  sync
...
MODEL_WEIGHTS='openai/gpt-oss-20b'
...
export DIXIE_BAKEOFF_GPT_OSS_API_BASE=http://127.0.0.1:8000/v1
mkdir -p output
for repetition in 1 2 3; do
  .venv/bin/dixie bakeoff configs/model_bakeoff.yaml \
    --candidate gpt-oss-20b \
    --output-dir "output/gpt-oss-20b/run-${repetition}"
done
